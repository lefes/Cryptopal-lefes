...
def xored(string1, string2):
    size = len(string1) if string1 < string2 else len(string2)
    result = bytearray(size)
    for i in range(size):
        result[i] = string1[i] ^ string2[i]
    return binascii.hexlify(result)

def xor_enc_key(file, key=b'ICE'):
    message = bytearray(open(file, 'rb').read())
    while len(key) < len(message):
        key += key
    print(xored(key, message))

if __name__ == '__main__':
    xor_enc_key('5_text.txt')
